...
async function show(opta_id){
  ...
  previousMatchDayId = json.data.competitions[0].matchdays.previous.id;
  nextMatchdayId = json.data.competitions[0].matchdays.next.id;
...
//-------------------------------------------------
async function getStandings_(competitionId){
  let standings_var = `?operationName=LiveTable&variables={"filter":{"idType":"COMPETITION","id":"sdc_fs-${competitionId}"}}`;
  let route = apiUrl+standings_var+extensionsS;
  const response = await(fetch(route));
  const json = await(response.json());
  //const standings = json.data.competitions[0].standings[0].standingsCollection;
  json.data.competitions[0].standings.forEach(element => {
    //console.log(element);
    standingsTableBody.innerHTML = element.standingsCollection.map(drawStandingsRow).join("\n");
    
  });
  //standingsTableBody.innerHTML = standings.map(drawStandingsRow).join("\n");
...
function drawStandingsRow(row){
  console.log(row.team.name.full);
  return `<tr><td>${row.position}</td><td>${row.team.name.full}</td><td>${row.points}</td></tr>`;
}
//----------------------------------------------------------------------------
async function getResults_(matchDayId){
  matchdayContainer.innerHTML = "loading";
  let matchday_var =`?operationName=CurrentCompetition&variables={"competitionsFilter":{"idType":"MATCHDAY","id":"${matchDayId}"}}`;
  let route = apiUrl+matchday_var+extensionsR;
  ...
  let maxIndex = allMatchdays.length-1;
    ...
    let results = json.data.competitions[0].sportEvents;
    resultsTableBody.innerHTML=results.map(drawResultRow).join('\n');
}
//----------------------------------------------------------------------------------
